    ...
    <div className="space-y-8 pb-20">
      ...
      {showCustomerModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-background dark:bg-card rounded-[2rem] border border-border shadow-2xl w-full max-w-md p-6 z-10">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-extrabold text-foreground">Select Customer</h3>
              <button onClick={() => setShowCustomerModal(false)} className="p-1.5 rounded-lg hover:bg-muted transition-colors text-muted-foreground">
                <X size={18} />
              </button>
            </div>
            <div className="relative mb-4">
              ...
              <input
                ...
                className="w-full bg-muted/40 border border-border/50 rounded-xl pl-10 pr-4 py-3 text-sm font-bold text-foreground focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none transition-all"
                autoFocus
              />
            </div>
            <div className="space-y-2 max-h-72 overflow-y-auto">
              {filteredCustomers.map((c) => (
                <button
                  ...
                  className="w-full text-left p-4 rounded-xl hover:bg-muted transition-colors border border-transparent hover:border-border/50"
                >
        ...
        <div className="flex-1 space-y-8 w-full">
          ...
          <div className="bg-white dark:bg-card rounded-[2rem] border border-border/50 shadow-sm p-6 lg:p-8">
            ...
            {cart.length > 0 && (
              <div className="space-y-3 mb-6">
                {cart.map((item) => (
                  <div
                    key={item.id}
                    className="bg-muted/30 border border-border/60 rounded-2xl p-4 flex flex-col sm:flex-row items-center gap-4"
                  >
                    <div className="flex items-center gap-4 flex-1 w-full">
                      <div className="w-14 h-14 rounded-xl bg-slate-900 flex items-center justify-center overflow-hidden shrink-0 shadow-inner">
                        <Image src={item.image} width={40} height={40} alt={item.name} className="object-cover drop-shadow-md brightness-110" />
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                      {/* Qty controls */}
                      <div className="flex items-center gap-2 bg-white dark:bg-card border border-border/60 rounded-xl px-3 py-2 shadow-sm">
                        <button
                          onClick={() => updateQty(item.id, -1)}
                          className="w-6 h-6 flex items-center justify-center rounded-lg bg-muted hover:bg-muted/80 transition-colors text-foreground"
                        >
                          <Minus size={12} />
                        </button>
            ...
            <div className="relative">
              <div className="border-2 border-dashed border-border/60 rounded-2xl p-6">
                ...
                {showProductResults && filteredProducts.length > 0 && (
                  <div className="mt-3 bg-white dark:bg-card border border-border/50 rounded-2xl shadow-xl overflow-hidden max-w-sm mx-auto">
          ...
          <div className="bg-green-50/50 dark:bg-green-950/10 rounded-[2rem] border border-green-100 dark:border-green-900/30 shadow-sm p-6 lg:p-8 flex flex-col">
            ...
            <div className="space-y-3 mt-auto">
              <button
                ...
                className={`w-full bg-green-700 hover:bg-green-800 text-white py-4 rounded-xl font-extrabold text-base transition-all shadow-xl shadow-green-700/20 hover:scale-105 active:scale-95 ${
              ...
              <button
                ...
                disabled={isSubmitting}
                className="w-full bg-green-100 dark:bg-green-900/30 text-green-900 dark:text-green-100 hover:bg-green-200 dark:hover:bg-green-900/50 py-4 rounded-xl font-extrabold text-sm transition-colors text-center disabled:opacity-50"
              >
                Discard Draft
              </button>
            </div>
