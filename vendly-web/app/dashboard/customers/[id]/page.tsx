  ...
  return (
    <div className="space-y-6 max-w-[1280px] mx-auto pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <button 
            ...
            className="flex-1 sm:flex-none px-5 py-2.5 rounded-xl bg-muted/40 hover:bg-muted text-foreground text-sm font-bold border border-border/50 transition-colors shadow-sm"
          >
            Send Email
          </button>
          <button 
            onClick={() => toast.success("Edit Profile modal opened")}
            className="flex-1 sm:flex-none px-5 py-2.5 rounded-xl bg-green-700 hover:bg-green-800 text-white text-sm font-bold shadow-md shadow-green-700/20 transition-colors active:scale-95"
          >
            Edit Profile
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Main Profile Info */}
        <div className="lg:col-span-2 bg-card border border-border/50 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center gap-6 lg:gap-8 shadow-sm">
          <div className="relative shrink-0">
            <div className="w-28 h-28 md:w-36 md:h-36 rounded-3xl overflow-hidden border-2 border-border/50 bg-slate-100 dark:bg-slate-800">
        ...
        <div className="bg-muted/20 border border-border/50 rounded-3xl p-6 md:p-8 flex flex-col justify-center shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="w-12 h-12 rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 flex items-center justify-center shrink-0">
               <Bot size={22} />
            </div>
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="space-y-4">
             {profile.orderHistory.map((order, i) => (
               <div 
                 ...
                 className="bg-card border border-border/50 rounded-2xl p-5 flex items-center justify-between shadow-sm hover:shadow-md transition-shadow group cursor-pointer"
               >
                 <div className="flex items-center gap-4">
                   <div className="w-12 h-12 rounded-xl bg-muted/40 border border-border/50 text-muted-foreground flex items-center justify-center group-hover:bg-muted transition-colors">
                     <Package size={20} />
                   </div>
                   <div>
        ...
        <div className="space-y-6 pt-2">
          ...
          <div className="bg-card border border-border/50 rounded-3xl p-6 lg:p-7 shadow-sm">
                ...
                {profile.preferences.map((pref, i) => (
                  <div key={i} className="space-y-2">
                    ...
                    <div className="w-full h-1.5 bg-muted rounded-full overflow-hidden">
                       <div 
                         className="h-full bg-green-700 dark:bg-green-600 rounded-full" 
          ...
          <div className="bg-card border border-border/50 rounded-3xl p-6 lg:p-7 shadow-sm">
             ...
             <div className="space-y-4 mb-4 max-h-[300px] overflow-y-auto pr-2">
               {savedNotes.map((note, idx) => (
                 <div key={idx} className="bg-muted/30 border border-border/50 rounded-2xl p-5">
             ...
             <div className="relative mt-2">
               <textarea 
                 ...
                 className="w-full bg-transparent border border-border/50 rounded-2xl p-4 text-sm font-medium text-foreground outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all resize-none h-24 placeholder:text-muted-foreground/50 pb-12"
               />
               <button 
                 onClick={handleAddNote}
                 disabled={!newNote.trim()}
                 className="absolute bottom-3 right-3 bg-green-700 hover:bg-green-800 text-white px-4 py-1.5 rounded-lg text-xs font-bold transition-all shadow-md shadow-green-700/20 active:scale-95 disabled:opacity-50"
               >
                 Save
               </button>
             </div>
...
function MetricCard({ label, value, subText, icon, trendColor }: { label: string, value: string, subText: string, icon?: React.ReactNode, trendColor?: string }) {
  return (
    <div className="bg-card border border-border/50 rounded-3xl p-6 lg:p-7 shadow-sm hover:shadow-md transition-shadow">
