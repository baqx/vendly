...
export default function CustomersPage() {
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    switch (status) {
      case "VIP":
        return (
          <span className="px-3 py-1 rounded-full bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400 text-[11px] font-extrabold tracking-widest uppercase">
            VIP
          </span>
        );
      case "ACTIVE":
        return (
          <span className="px-3 py-1 rounded-full bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400 text-[11px] font-extrabold tracking-widest uppercase border border-emerald-100 dark:border-emerald-800/30">
            ACTIVE
          </span>
        );
      case "NEW":
        return (
          <span className="px-3 py-1 rounded-full bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 text-[11px] font-extrabold tracking-widest uppercase">
            NEW
          </span>
        );
      default:
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="bg-card border border-border/50 rounded-2xl p-4 shadow-sm min-w-[200px] flex items-center justify-between">
          <div>
          ...
          <div className="w-12 h-12 rounded-xl bg-green-50 dark:bg-green-900/20 flex items-center justify-center text-green-700 dark:text-green-500 shrink-0">
            <Users size={24} />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex gap-1 bg-card p-1 rounded-xl border border-border/50 shadow-sm overflow-x-auto w-full sm:w-auto">
          {(["All Customers", "VIP Only", "New Leads"] as TabType[]).map((tab) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${
                activeTab === tab
                  ? "bg-muted/50 text-green-700 dark:text-green-400 shadow-sm"
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <button 
            ...
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-card border border-border hover:bg-muted px-4 py-2.5 rounded-xl text-sm font-bold text-foreground transition-all shadow-sm"
          >
            <Filter size={16} /> Filters
          </button>
          <Link 
            href="/dashboard/customers/add"
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-green-700 hover:bg-green-800 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-md shadow-green-700/20"
          >
      ...
      {/* ── Table Container ── */}
      <div className="bg-card border border-border/50 rounded-3xl shadow-sm overflow-hidden pt-2 relative">
                ...
                >
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-800 overflow-hidden shrink-0 border border-border">
        ...
        <div className="px-6 py-5 border-t border-border/40 flex flex-col sm:flex-row items-center justify-between gap-4 bg-muted/10 dark:bg-transparent">
          ...
          <div className="flex items-center gap-1">
            <button className="w-8 h-8 flex items-center justify-center rounded-lg border border-border bg-card text-muted-foreground hover:bg-muted/50 hover:text-foreground transition-colors disabled:opacity-50">
              <ChevronLeft size={14} />
            </button>
            <button className="w-8 h-8 flex items-center justify-center rounded-lg bg-green-700 text-white font-extrabold text-xs shadow-md shadow-green-700/20">
              1
            </button>
            <button className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-muted transition-colors text-foreground font-bold text-xs">
              2
            </button>
            <button className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-muted transition-colors text-foreground font-bold text-xs">
              3
            ...
            <button className="w-8 h-8 flex items-center justify-center rounded-lg border border-border bg-card text-muted-foreground hover:bg-muted/50 hover:text-foreground transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
        ...
        {/* Engagement Split */}
        <div className="bg-muted/30 border border-border/50 rounded-3xl p-6 lg:p-8">
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="w-full bg-slate-200 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                <div className="bg-green-700 h-full rounded-full w-[64%]" />
              </div>
            </div>
            <div>
              <div className="flex justify-between items-center mb-2">
              ...
              <div className="w-full bg-slate-200 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                <div className="bg-slate-400 dark:bg-slate-500 h-full rounded-full w-[36%]" />
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Automated Retention */}
        <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 rounded-3xl p-6 lg:p-8 flex items-center justify-between gap-6 relative overflow-hidden">
          ...
          <div className="w-20 h-20 rounded-2xl bg-[#e3ecf8] dark:bg-blue-900/40 flex items-center justify-center shrink-0 shadow-inner relative z-10">
