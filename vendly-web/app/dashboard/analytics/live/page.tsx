...
function LiveRevenueCard({ className }: { className?: string }) {
  return (
    <div className={`bg-card rounded-[4px] p-8 relative overflow-hidden shadow-sm border border-border/40 ${className}`}>
      <div className="flex flex-col h-full relative z-10">
        ...
        <div className="flex items-center gap-4 mt-auto pt-8">
          <div className="flex items-center gap-1 px-3 py-1.5 bg-success-bg text-green-700 dark:bg-green-900/30 dark:text-green-400 rounded-[4px] text-xs font-black tracking-wide">
...
function LiveNowCard({ className }: { className?: string }) {
  return (
    <div className={`bg-[#0f5a34] dark:bg-[#0a4528] rounded-[4px] p-8 text-white relative shadow-md ${className}`}>
      <div className="flex flex-col h-full relative z-10">
        ...
        <div className="space-y-6 mt-auto pt-4">
          <div className="space-y-2">
            ...
            <div className="h-[4px] bg-green-900/60 rounded-[4px] overflow-hidden">
              <div className="h-full bg-white w-[65%] rounded-[4px] shadow-[0_0_10px_rgba(255,255,255,0.5)]" />
            </div>
          </div>
          <div className="space-y-2">
            <div className="flex justify-between text-xs font-bold text-green-100">
            ...
            <div className="h-[4px] bg-green-900/60 rounded-[4px] overflow-hidden">
              <div className="h-full bg-green-400 w-[35%] rounded-[4px] shadow-[0_0_10px_rgba(74,222,128,0.5)]" />
            </div>
          </div>
        </div>
      </div>
...
function MapCard({ className }: { className?: string }) {
  ...
  return (
    <div className={`bg-card rounded-[4px] shadow-sm border border-border overflow-hidden flex flex-col min-h-[400px] ${className}`}>
      <div className="p-6 flex items-center justify-between border-b border-border bg-card z-20 relative">
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800 rounded-[4px] p-1 gap-1 relative border border-border shadow-inner">
              ...
              <div className="absolute top-full left-0 mt-1 w-32 bg-white dark:bg-slate-800 border border-border rounded-[4px] shadow-xl py-1 z-50 overflow-hidden">
                {regions.map((r) => (
                  <button
                    ...
                    className={`block w-full text-left px-4 py-2 text-[10px] font-bold uppercase tracking-wider transition-colors hover:bg-success-bg dark:hover:bg-slate-700 ${view === r ? 'text-green-700 dark:text-green-400 bg-success-bg/50 dark:bg-slate-700/50' : 'text-foreground dark:text-slate-300'}`}
                  >
                    {r.replace("-", " ")}
                  </button>
                ))}
...
function RecentFeedCard({ className }: { className?: string }) {
  return (
    <div className={`bg-slate-50 dark:bg-card border border-border/40 rounded-[4px] shadow-sm flex flex-col ${className}`}>
      ...
      <div className="px-6 pb-6 space-y-3 flex-1 overflow-y-auto">
        {RECENT_FEED.map((item) => (
          <div key={item.id} className={`bg-white dark:bg-slate-800/50 p-4 rounded-[4px] shadow-sm flex gap-4 ${item.border}`}>
            <div className={`w-10 h-10 rounded-[4px] flex items-center justify-center shrink-0 ${item.bgIcon}`}>
              <item.icon size={18} />
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                ...
                {item.badge && (
                  <span className="px-2 py-0.5 bg-success-bg text-green-700 dark:bg-green-900/30 dark:text-green-400 text-[10px] font-bold rounded">
                    {item.amount}
                  </span>
                )}
              </div>
...
function TransactionPulseCard({ className }: { className?: string }) {
  return (
    <div className={`bg-card border border-border/40 rounded-[4px] p-8 shadow-sm flex flex-col min-h-[350px] ${className}`}>
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-4 hidden sm:flex">
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-[4px] bg-[#0f5a34]" />
            <span className="text-[10px] font-bold text-muted-foregrounduppercase tracking-widest">Sales</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-[4px] bg-green-200 dark:bg-green-800" />
        ...
        {PULSE_DATA.map((d, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-1 h-full justify-end group">
             <div className="w-full flex flex-col gap-0.5 h-full justify-end">
                <div 
                  className={`w-full rounded-[4px] mt-auto transition-all duration-300 ${d.highlight ? 'bg-[#0f5a34]' : 'bg-[#0f5a34]/30 dark:bg-[#0f5a34]/60 group-hover:opacity-80'}`}
                  style={{ height: d.sales + '%' }}
                />
                <div 
                  className={`w-full rounded-[4px] transition-all duration-300 ${d.highlight ? 'bg-green-400' : 'bg-green-200 dark:bg-green-800/60 group-hover:opacity-80'}`}
...
function CriticalAlertsCard({ className }: { className?: string }) {
  return (
    <div className={`bg-card border border-border/40 rounded-[4px] p-6 shadow-sm flex flex-col ${className}`}>
      <h3 className="text-sm font-bold text-foreground mb-6">Critical Alerts</h3>
      
      <div className="space-y-3 flex-1">
        <div className="p-4 rounded-[4px] bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 flex gap-4">
          <div className="w-10 h-10 rounded-[4px] bg-red-100 dark:bg-red-900/30 text-red-600 flex items-center justify-center shrink-0">
        ...
        <div className="p-4 rounded-[4px] bg-slate-50 dark:bg-slate-800/50 border border-border/50 flex gap-4">
          <div className="w-10 h-10 rounded-[4px] bg-slate-200 dark:bg-slate-700 text-muted-foreground flex items-center justify-center shrink-0">
            <Info size={20} />
          </div>
          <div>
      ...
      <button 
        ...
        className="mt-6 w-full py-3 bg-info-bg dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 text-[11px] font-bold uppercase tracking-wider rounded-[4px] hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors flex items-center justify-center gap-2"
      >
        View All Alerts
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
             <div className="w-2 h-2 bg-emerald-500 rounded-[4px]" />
             <p className="text-sm text-muted-foregroundfont-medium">Real-time system monitoring active</p>
          </div>
        </div>
        
        <div className="bg-card border border-border/40 px-4 py-2 rounded-[4px] flex items-center gap-3 shadow-sm">
          <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Status</span>
          <span className="text-[10px] font-bold uppercase tracking-wider text-green-700 dark:text-green-500 bg-success-bg dark:bg-green-900/30 px-2 py-0.5 rounded">Operational</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 flex h-full relative">
          ...
          <button 
            ...
            className="absolute bottom-6 right-6 w-12 h-12 bg-[#0f5a34] justify-center text-white rounded-[4px] flex items-center shadow-lg hover:bg-green-800 transition-colors z-20"
          >
            <Plus size={24} />
          </button>
        </div>
