...
export default function InventoryDetailsPage({ params }: { params: { id: string } }) {
  ...
  if (deleted) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <Link href="/dashboard/inventory" className="mt-4 bg-green-700 text-white px-8 py-3 rounded-xl font-bold hover:bg-green-800 transition-colors shadow-lg">
          Back to Inventory
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-8 pb-16 relative">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-card rounded-3xl p-8 max-w-md w-full shadow-2xl border border-border/50">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowDeleteModal(false)} className="p-2 hover:bg-muted rounded-xl transition-colors text-muted-foreground">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteModal(false)} className="flex-1 py-3 rounded-xl border border-border font-bold text-foreground hover:bg-muted transition-colors">
                Cancel
              </button>
              <button onClick={handleDelete} className="flex-1 py-3 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold transition-colors shadow-lg active:scale-95">
                Yes, Delete
              </button>
            </div>
          </div>
      ...
      {showReviewModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-card rounded-3xl p-8 max-w-lg w-full shadow-2xl border border-border/50 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6 sticky top-0 bg-white dark:bg-card pb-2 z-10">
              ...
              <button onClick={() => setShowReviewModal(false)} className="p-2 hover:bg-muted rounded-xl transition-colors text-muted-foreground">
                <X size={20} />
              </button>
            </div>
            <div className="space-y-5">
              {[
                ...
                <div key={i} className="p-4 bg-muted/30 rounded-2xl border border-border/40">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-muted/50 hover:bg-red-50 dark:hover:bg-red-950/40 text-muted-foreground hover:text-red-600 rounded-xl font-bold transition-all text-sm border border-transparent hover:border-red-100 dark:hover:border-red-900/50"
          >
            <Trash2 size={16} />
            <span>Delete</span>
          </button>
          <Link
            ...
            className="bg-green-700 hover:bg-green-800 text-white px-6 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-green-700/20 hover:scale-105 active:scale-95 text-sm"
          >
            <Edit2 size={16} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col space-y-6 pt-2">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="bg-green-500 text-white px-3 py-1 rounded-full text-[10px] font-extrabold tracking-wider uppercase flex items-center gap-1 shadow-sm">
              ✓ In Stock
            </span>
          </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              {SIZES.map((size) => (
                <button
                  ...
                  className={`px-6 py-3 rounded-2xl text-sm font-extrabold transition-all active:scale-95 shadow-sm ${
                    selectedSize === size
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-12">
        ...
        <div className="bg-white dark:bg-card rounded-[2rem] p-8 border border-border/50 shadow-sm flex flex-col h-full relative overflow-hidden">
        ...
        <div className="bg-white dark:bg-card rounded-[2rem] p-8 border border-border/50 shadow-sm flex flex-col h-full relative">
          <button
          ...
          <div className="mt-10 max-w-sm">
            <button
              onClick={() => setShowReviewModal(true)}
              className="w-full py-4 rounded-xl border-2 border-border/80 font-extrabold text-sm text-foreground hover:bg-muted/50 hover:border-border transition-colors shadow-sm active:scale-[0.98]"
            >
      ...
      <div className="mt-10">
        ...
        <div className="bg-white dark:bg-card rounded-[2rem] p-3 border border-border/50 shadow-sm">
          {[
            {
              id: 0,
              icon: <Truck size={24} />,
            ...
            <div key={item.id}>
              ...
              {expandedLogistic === item.id && (
                <div className="mx-5 mb-4 p-5 bg-muted/30 rounded-2xl border border-border/40 text-sm font-medium text-muted-foreground leading-relaxed">
                  {item.detail}
                </div>
              )}
